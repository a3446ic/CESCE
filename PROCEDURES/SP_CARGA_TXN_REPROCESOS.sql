...
/*---------------------------------------------------------------------
    ...
    | Version: 06	2024-12-24  Si los campos GENERICATTRIBUTE6 y/o GENERICATTRIBUTE7 son NULL se deben calcular los valores
    |                             GENERICATTRIBUTE6 --> (SELECT EXT.LIB_GLOBAL_CESCE:getCodGarantia(COD_OPERACION, COD_RIESGO) FROM DUMMY)
    |                             GENERICATTRIBUTE7 --> (SELECT EXT.LIB_GLOBAL_CESCE:getCueRiesgo(COD_RIESGO) FROM DUMMY)
    |----------------------------------------------------------------------
...
--		TX.GENERICATTRIBUTE3 AS GENERICATTRIBUTE3, 
		...
		CASE WHEN TX.GENERICATTRIBUTE6 IS NULL THEN (SELECT EXT.LIB_GLOBAL_CESCE:getCodGarantia(TR.ORDERID, COD_RIESGO) FROM DUMMY) ELSE TX.GENERICATTRIBUTE6 END AS GENERICATTRIBUTE6, 
		CASE WHEN TX.GENERICATTRIBUTE7 IS NULL THEN (SELECT EXT.LIB_GLOBAL_CESCE:getCueRiesgo(COD_RIESGO) FROM DUMMY) ELSE TX.GENERICATTRIBUTE7 END AS GENERICATTRIBUTE7, 
